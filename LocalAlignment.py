def initialize_local_mat(s1, s2):
...
def findLocalAlignment(s1, s2):
    bt_dict = {0:'diag', 1:'left', 2:'up', 3:'source'}
    indelPen = 5
    dp_mat = initialize_local_mat(s1, s2)
    ...
    for i in range(1, len(dp_mat)):
        for j in range(1, len(dp_mat[0])):
            ...
            values = [dp_mat[i-1][j-1] + var, dp_mat[i][j-1] - indelPen, dp_mat[i-1][j] - indelPen, 0]
    ...
    return backtrack_global(startPoint, bt_mat, s1, s2)


def backtrack_global(startPoint, bt_mat, s1, s2):
    s1align = ''
    s2align = ''
    ...
    while bt_mat[i][j] != 'source' and i != 0 and j != 0:
        if bt_mat[i][j] == 'diag':
            s1align += s1[j-1]
            s2align += s2[i-1]
...
with open('test.txt','r+') as file:
    ...
    alignment = findLocalAlignment(s1, s2)
    print('\n'.join(alignment))
