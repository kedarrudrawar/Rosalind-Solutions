def initialize_overlap_mat(s1, s2, indelPen):
...
def findOverlapAlignment(s1, s2):
    bt_dict = {0:'diag', 1:'left', 2:'up'}
    indelPen = 2
    misMatchPen = 2
    matchReward = 1
    dp_mat = initialize_overlap_mat(s1, s2, indelPen)
    ...
    for i in range(1, len(dp_mat)):
        for j in range(1, len(dp_mat[0])):
            var = matchReward if s1[i-1] == s2[j-1] else -misMatchPen
...
with open('rosalind_ba5i.txt','r+') as file:
    ...
    alignment = findOverlapAlignment(s1, s2)
    print('\n'.join(alignment))
